...
// --- HÀM TOÀN CỤC (CÓ TÍCH HỢP CHAT) ---
window.HR_Global = {
    // 1. Gửi tin nhắn tự động vào Chat
    logToChat: async (userName, message) => {
        try {
            await addDoc(collection(db, `${ROOT_PATH}/chat`), {
                user: userName,
                message: message,
                time: Date.now() // Chat sẽ tự sắp xếp theo thời gian
            });
        } catch (e) { console.error("Lỗi chat tự động:", e); }
    },

    // 2. Các tác vụ công việc
    delTask: (id) => { if(confirm("Xóa việc này?")) deleteDoc(doc(db, `${ROOT_PATH}/tasks`, id)); },
    
    accept: async (id, title, userName) => { 
        await updateDoc(doc(db, `${ROOT_PATH}/tasks`, id), { status: 'DOING' });
        window.HR_Global.logToChat(userName, `💪 Đã nhận việc: "${decodeURIComponent(title)}"`);
    }, 
    
    finish: async (id, title, userName) => { 
        await updateDoc(doc(db, `${ROOT_PATH}/tasks`, id), { status: 'DONE' });
        window.HR_Global.logToChat(userName, `✅ Đã báo cáo xong: "${decodeURIComponent(title)}"`);
    },
    ...
    checkIn: async (userName) => {
        if(confirm("Chấm công hôm nay?")) {
            ...
            window.HR_Global.logToChat(userName, "📍 Đã chấm công có mặt!");
        }
    },
    reqLeave: async (userName) => {
        ...
        if(r) {
            ...
            window.HR_Global.logToChat(userName, `📝 Đã gửi đơn xin nghỉ: "${r}"`);
        }
    },
    reqBuy: async (userName) => {
        ...
        if(n) {
            ...
            window.HR_Global.logToChat(userName, `🛒 Đề xuất mua: "${n}"`);
        }
    },
    
    // 4. Thưởng phạt (Chỉ Admin)
    adjustScore: async (id, name, val, adminName) => {
        const reason = prompt("Lý do điều chỉnh điểm?");
        if(reason) {
            ...
            const icon = val > 0 ? "🎁" : "⚠️";
            window.HR_Global.logToChat(adminName, `${icon} Đã ${type} ${Math.abs(val)} điểm cho ${name}. Lý do: ${reason}`);
        }
    }
};
    ...
    renderTasks: (data, user) => {
        ...
        const renderList = () => {
            ...
            container.innerHTML = filtered.length ? filtered.map(t => {
                ...
                const safeTitle = encodeURIComponent(t.title); // Mã hóa để truyền vào hàm không lỗi
                ...
                <div class="bg-white p-3 rounded-lg border-l-4 ${isDoing?'border-blue-500 bg-blue-50':(isDone?'border-green-500 opacity-60':'border-orange-400')} shadow-sm relative">
                    ...
                    <div class="mt-2 pt-2 border-t border-dashed border-slate-100">
                        ${!isDoing ? 
                            `<button onclick="window.HR_Global.accept('${t.id}', '${safeTitle}', '${user.name}')" class="w-full py-1.5 bg-blue-100 text-blue-700 font-bold text-[10px] rounded hover:bg-blue-200">NHẬN VIỆC</button>` : 
                            `<button onclick="window.HR_Global.finish('${t.id}', '${safeTitle}', '${user.name}')" class="w-full py-1.5 bg-green-100 text-green-700 font-bold text-[10px] rounded hover:bg-green-200">BÁO CÁO XONG</button>`
                        }
                    </div>` : ''}
        ...
        setTimeout(() => {
            ...
            if(btn) btn.onclick = async () => {
                ...
                // Chat thông báo giao việc
                const nameStr = names.length > 3 ? `${names.length} người` : names.join(', ');
                window.HR_Global.logToChat(user.name, `📢 Đã giao việc: "${title}" cho ${nameStr}`);
            ...
            <div class="glass p-4 bg-purple-50/50 border border-purple-100 shadow-sm rounded-xl">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <button onclick="window.HR_Global.checkIn('${user.name}')" class="p-3 bg-white rounded-xl border border-purple-200 shadow-sm flex flex-col items-center active:scale-95 transition hover:shadow-md">
                        <i class="fas fa-fingerprint text-2xl text-blue-500 mb-2"></i><span class="text-[10px] font-bold text-slate-600">Điểm danh</span>
                    </button>
                    <button onclick="window.HR_Global.reqLeave('${user.name}')" class="p-3 bg-white rounded-xl border border-purple-200 shadow-sm flex flex-col items-center active:scale-95 transition hover:shadow-md">
                        <i class="fas fa-user-clock text-2xl text-orange-500 mb-2"></i><span class="text-[10px] font-bold text-slate-600">Xin nghỉ</span>
                    </button>
                    <button onclick="window.HR_Global.reqBuy('${user.name}')" class="p-3 bg-white rounded-xl border border-purple-200 shadow-sm flex flex-col items-center active:scale-95 transition hover:shadow-md">
                        <i class="fas fa-shopping-cart text-2xl text-green-500 mb-2"></i><span class="text-[10px] font-bold text-slate-600">Mua hàng</span>
                    </button>
                </div>
            </div>
                ...
                <div class="space-y-2">
                    ...
                    <div class="bg-white p-3 rounded-lg shadow-sm border border-slate-100 flex justify-between items-center">
                        <div class="flex items-center gap-3">
                            <div class="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center font-black text-slate-500 border border-slate-200 shadow-inner">${e.name.charAt(0)}</div>
                        ...
                        <div class="flex items-center gap-3">
                            <div class="text-right"><div class="font-black text-xl ${Number(e.score)>=0 ? 'text-green-600' : 'text-red-500'}">${e.score || 0}</div><div class="text-[8px] text-slate-400 uppercase font-bold">Điểm</div></div>
                            ${isAdmin ? `<div class="flex flex-col gap-1"><button onclick="window.HR_Global.adjustScore('${e._id}', '${e.name}', 10, '${user.name}')" class="w-6 h-6 bg-green-100 text-green-700 rounded-lg flex items-center justify-center hover:bg-green-200 font-bold shadow-sm transition active:scale-90">+</button><button onclick="window.HR_Global.adjustScore('${e._id}', '${e.name}', -10, '${user.name}')" class="w-6 h-6 bg-red-100 text-red-700 rounded-lg flex items-center justify-center hover:bg-red-200 font-bold shadow-sm transition active:scale-90">-</button></div>` : ''}
                        </div>
                    </div>`).join('')}
                </div>
            </div>
