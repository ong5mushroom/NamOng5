...
window.HR_Action = {
    // Chat tự động
    chat: async (user, msg) => { 
        try { await addDoc(collection(db, `${ROOT_PATH}/chat`), {user, message:msg, time:Date.now()}); } catch(e){} 
    },
    ...
    // Tiện ích cá nhân (Team)
    checkIn: async (u) => { 
        if(confirm("Xác nhận chấm công?")) { 
            await addDoc(collection(db, `${ROOT_PATH}/tasks`), {title:"Đã chấm công", to:"ADMIN", by:u, type:"CHECKIN", status:"PENDING", time:Date.now()}); 
...
export const HR = {
    ...
    renderTasks: (data, user) => {
        ...
        const renderList = () => {
            ...
            // Lọc: Chỉ lấy TASK (không lấy đơn từ), theo người, theo quyền
            ...
            filtered.sort((a,b) => b.time - a.time); // Mới nhất lên đầu
        ...
        // GẮN SỰ KIỆN NÚT GIAO VIỆC (ĐÃ FIX LỖI "KHÔNG ĂN")
        setTimeout(() => {
            renderList();
            ...
            if(btn) {
                ...
                document.getElementById('btn-add-task').onclick = async function() {
                    ...
                    if(checked.length === 0) return Utils.toast("Chưa chọn nhân viên!", "err");

                    // Hiệu ứng đang gửi
                    me.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Đang gửi...';
                    me.disabled = true;
                    ...
                    try {
                        ...
                        window.HR_Action.chat(user.name, `📢 Đã giao việc: "${title}" cho ${nameStr}`);
                        ...
                        // Trả lại nút bấm
                        me.innerHTML = 'GIAO VIỆC NGAY';
                        me.disabled = false;
                    }
                };
            }
        }, 300); // Tăng thời gian chờ load DOM lên chút
    },


    // --- TAB 2: TEAM & BẢNG XẾP HẠNG ---
    renderTeam: (data, user) => {
        ...
        // SẮP XẾP BẢNG XẾP HẠNG (Điểm cao lên đầu)
        ...
        <div class="space-y-5 pb-24">
            <div class="glass p-4 bg-purple-50/50 border border-purple-100 shadow-sm rounded-xl">
                ...
                <div class="grid grid-cols-3 gap-3">
                    <button onclick="window.HR_Action.checkIn('${user.name}')" class="p-3 bg-white rounded-xl border border-purple-200 shadow-sm flex flex-col items-center active:scale-95 transition hover:shadow-md">
                        <i class="fas fa-fingerprint text-2xl text-blue-500 mb-2"></i><span class="text-[10px] font-bold text-slate-600">Điểm danh</span>
                    </button>
                    <button onclick="window.HR_Action.reqLeave('${user.name}')" class="p-3 bg-white rounded-xl border border-purple-200 shadow-sm flex flex-col items-center active:scale-95 transition hover:shadow-md">
                        <i class="fas fa-user-clock text-2xl text-orange-500 mb-2"></i><span class="text-[10px] font-bold text-slate-600">Xin nghỉ</span>
                    </button>
                    <button onclick="window.HR_Action.reqBuy('${user.name}')" class="p-3 bg-white rounded-xl border border-purple-200 shadow-sm flex flex-col items-center active:scale-95 transition hover:shadow-md">
            ...
            <div>
                <h2 class="font-black text-slate-700 text-sm uppercase pl-1 border-l-4 border-yellow-500 mb-3 flex items-center gap-2">
                    <i class="fas fa-trophy text-yellow-500"></i> BẢNG XẾP HẠNG THI ĐUA
                </h2>
                <div class="space-y-2">
                    ${sortedEmployees.map((e, idx) => {
                        const rankIcon = idx === 0 ? '🥇' : (idx === 1 ? '🥈' : (idx === 2 ? '🥉' : `${idx+1}.`));
                        const rankClass = idx < 3 ? 'bg-yellow-50 border-yellow-200' : 'bg-white border-slate-100';
                        
                        return `
                        <div class="${rankClass} p-3 rounded-lg shadow-sm border flex justify-between items-center">
                            <div class="flex items-center gap-3">
                                <div class="font-black text-lg w-8 text-center text-slate-500">${rankIcon}</div>
                                <div>
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="flex flex-col gap-1">
                                    <button onclick="window.HR_Action.adjustScore('${e._id}', '${e.name}', 10, '${user.name}')" class="w-6 h-6 bg-green-100 text-green-700 rounded-lg flex items-center justify-center hover:bg-green-200 font-bold shadow-sm active:scale-90">+</button>
                                    <button onclick="window.HR_Action.adjustScore('${e._id}', '${e.name}', -10, '${user.name}')" class="w-6 h-6 bg-red-100 text-red-700 rounded-lg flex items-center justify-center hover:bg-red-200 font-bold shadow-sm active:scale-90">-</button>
                                </div>` : ''}
                            </div>
                        </div>`;
                    }).join('')}
