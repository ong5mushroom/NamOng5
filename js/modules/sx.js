...
window.SX_Action = {
    ...
    // 1. RESET SẠCH SẼ (Fix lỗi còn sót mã)
    reset0: async (hid) => { 
        if(confirm("⚠️ XÓA TRẮNG NHÀ NÀY?\n(Hành động này sẽ xóa hết số lượng và mã lô)")) { 
            await updateDoc(doc(db, `${ROOT_PATH}/houses`, hid), { 
                batchQty: 0,
                currentBatch: '', // Xóa mã lô hiển thị
                importDate: null, // Xóa ngày nhập
                status: 'EMPTY'   // Tắt đèn
            }); 
            Utils.toast("✅ Đã Reset sạch sẽ!"); 
        } 
    },

    // 2. SỬA TAY (Tự động OFF nếu <= 0)
    adjust: async (hid, currentQty) => { 
        ...
        if(v) { 
            ...
            // Logic tự động tắt/mở
            if (newQty <= 0) {
                updateData.status = 'EMPTY';
                updateData.currentBatch = ''; // Hết hàng thì xóa mã luôn
