...
window.THDG_Action = {
    delOne: async (id, name) => { if(confirm(`Xóa ${name}?`)) { await deleteDoc(doc(db, `${ROOT_PATH}/products`, id)); Utils.toast("Đã xóa!"); } },
    
    // RESET CHUẨN (Dùng writeBatch và getDocs từ config mới)
    resetAll: async () => {
        if(confirm("⚠️ CẢNH BÁO: XÓA SẠCH DANH SÁCH SP?\n(Nhập OK để xóa)")) {
            try {
            ...
            <div id="zone-harvest" class="animate-pop">
                <div class="glass p-4 border-l-8 border-green-500 bg-green-50/40">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        ${isAdmin ? `<div class="flex gap-2"><button onclick="window.THDG_Action.resetAll()" class="bg-red-100 text-red-600 px-2 py-1 rounded text-[10px] font-bold border border-red-200 shadow-sm">RESET</button><button id="btn-add" class="bg-white border border-green-600 text-green-700 px-2 py-1 rounded text-[10px] font-bold shadow-sm">+ MÃ</button></div>` : ''}
                    </div>
                    <div class="space-y-4">
                        <div class="flex gap-2"><input type="date" id="h-date" class="w-1/3 p-2 rounded border text-xs font-bold bg-white"><select id="h-area" class="flex-1 p-2 rounded border text-xs font-bold bg-white"><option value="">-- Nguồn --</option>${(data.houses||[]).map(h=>`<option value="${h.id}" data-name="${h.name}">${h.name}</option>`).join('')}<option value="MuaNgoai" data-name="Mua Ngoài">Mua Ngoài</option></select></div>
                        ${Object.keys(groups).map(k => `<div class="bg-white/60 p-2 rounded-xl border border-slate-200"><div class="text-[10px] font-bold text-slate-500 mb-2 uppercase border-b border-slate-100 pb-1">${groups[k].title}</div><div class="grid grid-cols-1 gap-1">${groups[k].items.length ? groups[k].items.map(renderRow).join('') : '<div class="text-[10px] text-slate-300 italic text-center py-2">-- Trống --</div>'}</div></div>`).join('')}
                        <button id="btn-save-h" class="w-full py-3 bg-green-600 text-white rounded-lg font-bold text-xs shadow-lg active:scale-95 transition">LƯU KHO</button>
                    </div>
                </div>
            </div>
        ...
        setTimeout(() => {
            ...
            if(isAdmin) {
                ...
                if(btnAdd) btnAdd.onclick = () => {
                    Utils.modal("Tạo Mã Mới", `<input id="n-n" placeholder="Tên" class="w-full p-2 border rounded mb-2"><input id="n-c" placeholder="Mã" class="w-full p-2 border rounded mb-2"><select id="n-g" class="w-full p-2 border rounded"><option value="1">1. Tươi</option><option value="2">2. Phụ</option><option value="3">3. Sơ chế</option></select>`, [{id:'s-ok', text:'Lưu'}]);
            ...
            document.getElementById('btn-save-h').onclick = async () => {
                ...
                if(!dVal || !aid) return Utils.toast("Thiếu thông tin!", "err");
                const batch = writeBatch(db); // Dùng writeBatch
                let hasData = false; let totalKg = 0;
                products.forEach(p => { const el = document.getElementById(`in-${p.code}`); if(el && Number(el.value) > 0) { const q = Number(el.value); batch.update(doc(db, `${ROOT_PATH}/products`, p._id), {stock: increment(q)}); totalKg += q; el.value = ''; hasData = true; } });
                if(hasData) {
                    ...
                    batch.set(ref, {area: aname, time: new Date(dVal).setHours(12), user: user.name, total: totalKg});
                    await batch.commit(); Utils.toast(`Đã lưu ${totalKg}kg!`);
