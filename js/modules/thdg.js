...
export const THDG = {
    render: (data, user) => {
                    ...
                    <div class="space-y-3 bg-white p-3 rounded-xl shadow-sm border border-orange-100">
                        ...
                        <div class="bg-slate-50 p-2 rounded border border-slate-200">
                            <div class="flex gap-2 mb-2">
                                <select id="s-prod" class="flex-1 p-2 text-xs border rounded font-bold text-slate-700">
        ...
        const updateCart = () => {
            ...
            cart.forEach((_,i) => document.getElementById(`del-c-${i}`).onclick = () => { cart.splice(i,1); updateCart(); });
        };
        ...
        setTimeout(() => {
            ...
            if(btnSaveH) btnSaveH.onclick = async () => {
                const areaId = document.getElementById('h-area').value;
                if(!areaId) return Utils.toast("Chưa chọn nguồn thu!", "err");
                
                const areaName = document.getElementById('h-area').options[document.getElementById('h-area').selectedIndex].getAttribute('data-name');
                let details = {}, total = 0;
                ...
                // Cộng sản lượng vào Nhà (để hiển thị ở Home)
                if(areaId !== 'MuaNgoai') {
                    // Lưu ý: Cần chắc chắn field totalYield có trong db hoặc dùng set với merge
                    // Ở đây giả định field đã có hoặc tạo mới nếu chưa
                    // Cách an toàn nhất là đọc trước rồi ghi, nhưng increment nhanh hơn
                    batch.update(doc(db, `${ROOT_PATH}/houses`, areaId), { totalYield: increment(total) });
                }

                await batch.commit();
                Utils.toast(`✅ Đã thu hoạch ${total}kg`);
            };
            ...
            document.getElementById('btn-save-sell').onclick = async () => {
                ...
                cart = []; updateCart(); document.getElementById('s-cust').value = '';
            };
            ...
            document.getElementById('btn-report-day').onclick = () => {
                if(!todayLogs.length) return Utils.toast("Hôm nay chưa có đơn nào!", "err");
                let csv = "Khach,SP,SL,Gia,ThanhTien\n";
                todayLogs.forEach(l => l.items.forEach(i => csv += `${l.customer},${i.name},${i.qty},${i.price},${i.qty*i.price}\n`));
                const link = document.createElement("a"); link.href = encodeURI("data:text/csv;charset=utf-8,"+csv); link.download = `BC_Ngay_${new Date().toLocaleDateString('vi-VN').replace(/\//g,'-')}.csv`; link.click();
            };
