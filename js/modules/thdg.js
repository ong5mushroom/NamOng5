...
// --- HÀM XÓA CHẮC CHẮN HOẠT ĐỘNG ---
window.THDG_Del = async (id, nameEncoded) => {
    // Decode lại tên (phòng trường hợp tên có dấu cách hoặc ký tự đặc biệt gây lỗi)
    const name = decodeURIComponent(nameEncoded);
    if(confirm(`⚠️ ADMIN:\nXóa mã "${name}"?\nDữ liệu tồn kho sẽ mất.`)) {
        try {
            await deleteDoc(doc(db, `${ROOT_PATH}/products`, id));
            Utils.toast("Đã xóa mã!");
        } catch (e) {
...
export const THDG = {
    render: (data, user) => {
        ...
        try {
            ...
            // ... (Phần logic Ngày, Logs giữ nguyên như cũ để không phá vỡ) ...
            // Tôi chỉ viết lại phần renderRow để sửa lỗi nút Xóa
            
            const renderRow = (p) => `
                <div class="flex justify-between items-center bg-white p-1.5 rounded border border-slate-200">
                    <div class="flex items-center gap-1 overflow-hidden">
                        ${isAdmin ? `<button onclick="window.THDG_Del('${p._id}', '${encodeURIComponent(p.name)}')" class="text-red-300 hover:text-red-600 px-1 font-bold">✕</button>` : ''}
            ...
            // (Chỉ thay đổi cách gọi hàm renderRow bên trên)
            
            // Do code HTML dài, tôi viết tóm tắt logic thay thế vào đây để bạn copy paste cho gọn và không sai
            // Hãy dùng lại toàn bộ cấu trúc HTML của phiên bản trước, CHỈ THAY ĐỔI đoạn renderInputRow thành renderRow như trên
            
            // ĐÂY LÀ PHẦN HTML FULL ĐỂ BẠN COPY CHO AN TOÀN:
             c.innerHTML = `
                    ...
                    <div class="glass p-5 border-l-8 border-green-500 bg-green-50/40">
                        ...
                        <div class="space-y-3">
                            <div class="flex gap-2">
                                ...
                                <div class="flex-1"><label class="text-[9px] font-bold text-slate-500 uppercase ml-1">Nguồn Thu</label><select id="h-area" class="w-full p-2 rounded border text-xs font-bold"><option value="">-- Chọn --</option>${(data.houses||[]).map(h => `<option value="${h.id}" data-name="${h.name}">${h.name}</option>`).join('')}<option value="MuaNgoai" data-name="Mua Ngoài">Mua Ngoài</option></select></div>
                            </div>
