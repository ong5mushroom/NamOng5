...
// --- HÀM XÓA TOÀN CỤC (Đặt ngoài render để không bị lỗi undefined) ---
window.THDG_Actions = {
...
export const THDG = {
    render: (data, user) => {
        ...
        if (!data) { 
            c.innerHTML = '<div class="p-10 text-center text-slate-400">Đang tải...</div>'; 
            return; 
        }

        try {
            // PHÂN QUYỀN: Chỉ Admin/Quản lý mới thấy nút Xóa và nút Báo cáo
            ...
            // Hàm vẽ dòng nhập liệu (Có nút Xóa nếu là Admin)
            ...
            setTimeout(() => {
                ...
                document.getElementById('btn-save-h').onclick=async()=>{
                    ...
                    if(!dateVal||!aid) return Utils.toast("Thiếu Ngày/Nguồn!","err");
                ...
                document.getElementById('btn-save-sell').onclick=async()=>{
                    const cust=document.getElementById('s-cust').value;if(!cust||!cart.length)return Utils.toast("Thiếu tên/giỏ!","err");
                ...
                document.getElementById('btn-print').onclick=()=>{
                    ...
                    w.document.write(`<html><head><title>PHIEU BAN HANG</title><style>body{font-family:monospace;padding:10px;font-size:12px}.c{text-align:center}.r{text-align:right}table{width:100%;border-collapse:collapse;margin-top:10px}th{border-top:1px dashed #000;border-bottom:1px dashed #000}td{padding:4px 0}</style></head><body><div class="c"><h3>${COMPANY.name}</h3><p>${COMPANY.address}</p><p>MST:${COMPANY.mst} Hotline:${COMPANY.hotline}</p><br><h2>PHIẾU BÁN HÀNG</h2></div><p>Khách: <b>${document.getElementById('s-cust').value}</b></p><p>NV: ${user.name} - ${new Date().toLocaleString('vi-VN')}</p><table><tr><th align="left">SP</th><th class="c">SL</th><th class="r">Đơn giá</th><th class="r">Tiền</th></tr>${cart.map(i=>`<tr><td>${i.name}</td><td class="c">${i.qty}</td><td class="r">${i.price.toLocaleString()}</td><td class="r">${(i.qty*i.price).toLocaleString()}</td></tr>`).join('')}<tr><td colspan="3" class="r" style="border-top:1px dashed #000;padding-top:5px"><b>TỔNG CỘNG:</b></td><td class="r" style="border-top:1px dashed #000;padding-top:5px"><b>${cart.reduce((a,b)=>a+b.qty*b.price,0).toLocaleString()}</b></td></tr></table><div class="c" style="margin-top:20px"><i>${COMPANY.slogan}</i><br>Cảm ơn quý khách!</div></body></html>`);
