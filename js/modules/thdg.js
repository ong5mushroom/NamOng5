...
export const THDG = {
    render: (data, user) => {
        ...
        try {
            ...
            // Sắp xếp sản phẩm A-Z để vị trí cố định, không bị nhảy
            ...
            // Hàm tạo HTML dòng (Dùng class để bắt sự kiện, không dùng onclick window)
            const renderRow = (p) => `
                <div class="product-row flex justify-between items-center bg-white p-1.5 rounded border border-slate-200 mb-1" id="row-${p._id}">
                    <div class="flex items-center gap-1 overflow-hidden">
                        ${isAdmin ? `
                            <button class="btn-delete-item text-red-400 hover:text-red-600 px-2 font-bold text-lg leading-none" 
                                data-id="${p._id}" 
                                data-name="${p.name}" 
                                type="button">
                                ×
            ...
            <div class="space-y-5 pb-24">
                ...
                <div id="zone-harvest" class="animate-pop">
                    <div class="glass p-5 border-l-8 border-green-500 bg-green-50/40">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            ${isAdmin ? `<button id="btn-add-prod" class="bg-white border border-green-200 text-green-700 px-2 py-1 rounded text-[10px] font-bold shadow-sm">+ Mã SP</button>` : ''}
                        </div>
                        <div class="space-y-3">
                            ...
                            <div class="mt-4 pt-2 border-t border-green-200">
                                ...
                                <div class="max-h-32 overflow-y-auto space-y-1 mt-1 bg-white p-1 rounded border">${sortedHarvest.map(l=>`<div class="flex justify-between text-[10px] border-b border-dashed border-slate-100 pb-1 mb-1"><div><span class="text-slate-400 mr-1">${new Date(l.time).toLocaleDateString('vi-VN').slice(0,5)}</span><span class="font-bold text-slate-600">${l.area}</span></div><span class="font-bold text-green-600">+${l.total}kg</span></div>`).join('')}</div>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div id="zone-sell" class="hidden animate-pop">
                    <div class="glass p-5 border-l-8 border-orange-500 bg-orange-50/40">
                        ...
                        <div class="mt-4 pt-2 border-t border-orange-200"><span class="text-[9px] font-bold text-slate-400 uppercase">Đơn gần đây</span><div class="max-h-32 overflow-y-auto space-y-1 mt-1 bg-white p-1 rounded border">${sortedShips.map(s=>`<div class="flex justify-between text-[10px] border-b border-dashed border-slate-100 pb-1 mb-1"><div><span class="text-slate-400 mr-1">${new Date(s.time).toLocaleDateString('vi-VN').slice(0,5)}</span><span class="font-bold text-slate-700">${s.customer}</span></div><span class="font-bold text-orange-600">${Number(s.total).toLocaleString()}đ</span></div>`).join('')}</div></div>
                    </div>
                </div>
            </div>`;

            // 3. GẮN SỰ KIỆN (EVENT DELEGATION - FIX LỖI XÓA)
            setTimeout(() => {
                ...
                // --- XÓA MÃ SẢN PHẨM (Logic mới) ---
                if (isAdmin) {
                    // Gắn sự kiện click vào toàn bộ vùng Nhập Kho
                    const zoneHarvest = document.getElementById('zone-harvest');
                    if (zoneHarvest) {
                        zoneHarvest.onclick = async (e) => {
                            // Kiểm tra xem cái được click có phải nút xóa (hoặc con của nó) không
                            const btn = e.target.closest('.btn-delete-item');
                            if (btn) {
                                const id = btn.getAttribute('data-id');
                                const name = btn.getAttribute('data-name');
                                
                                if (confirm(`⚠️ XÓA VĨNH VIỄN mã: "${name}"?\n(Dữ liệu tồn kho sẽ mất)`)) {
                                    try {
                                        ...
                                        location.reload(); // Tải lại nếu lỗi để đồng bộ
                                    }
                                }
                            }
                        };
