...
// --- HÀM XÓA MÃ SẢN PHẨM (GẮN CỨNG WINDOW) ---
window.THDG_DelProd = async (id, name) => {
    // 1. Xác nhận kỹ
    if (confirm(`⚠️ CẢNH BÁO TUYỆT ĐỐI:\nBạn muốn XÓA VĨNH VIỄN mã: "${name}"?\n(Toàn bộ lịch sử tồn kho của mã này sẽ mất)`)) {
        try {
            // 2. Xóa ngay trên giao diện để không bấm nhầm lần 2
            const row = document.getElementById(`prod-row-${id}`);
            if (row) row.remove();

            // 3. Gửi lệnh xóa lên Server
            await deleteDoc(doc(db, `${ROOT_PATH}/products`, id));
            Utils.toast(`✅ Đã xóa mã: ${name}`);
        } catch (e) {
            alert("Lỗi xóa: " + e.message);
            location.reload(); // Tải lại nếu có lỗi để đồng bộ
        }
    }
};
...
export const THDG = {
    render: (data, user) => {
        ...
        try {
            ...
            // --- BẮT BUỘC: SẮP XẾP DANH SÁCH A->Z ĐỂ KHÔNG BỊ NHẢY DÒNG ---
            const products = (Array.isArray(data.products) ? data.products : []).sort((a, b) => (a.name || "").localeCompare(b.name || ""));
            const houses = Array.isArray(data.houses) ? data.houses : [];
            
            // Nhật ký & Đơn hàng (Mới nhất lên đầu, lấy 20 dòng)
            const sortedHarvest = [...(data.harvest || [])].sort((a,b) => b.time - a.time).slice(0, 20);
            const sortedShips = [...(data.shipping || [])].sort((a,b) => b.time - a.time).slice(0, 20);

            // Phân nhóm (Dữ liệu đã sort nên nhóm cũng sẽ sort)
            ...
            // HÀM RENDER DÒNG (Dùng ID cứng để xóa)
            const renderRow = (p) => `
                <div id="prod-row-${p._id}" class="flex justify-between items-center bg-white p-2 rounded border border-slate-200 mb-1 shadow-sm">
                    <div class="flex items-center gap-2 overflow-hidden">
                        ${isAdmin ? `
                            <button onclick="window.THDG_DelProd('${p._id}', '${p.name}')" class="w-6 h-6 flex items-center justify-center bg-red-50 text-red-500 rounded-full font-bold hover:bg-red-100 active:scale-90 transition">
                                ×
                            </button>` : ''}
                        <span class="text-[11px] font-bold text-slate-700 truncate w-28" title="${p.name}">${p.name}</span>
                    </div>
                    <input type="number" step="0.1" id="in-${p.code}" class="w-16 p-1.5 text-center font-bold text-slate-700 border border-slate-300 rounded text-xs focus:border-green-500 outline-none placeholder-slate-300" placeholder="0">
                </div>`;
            ...
            <div class="space-y-5 pb-24">
                ...
                <div id="zone-harvest" class="animate-pop">
                    <div class="glass p-5 border-l-8 border-green-500 bg-green-50/40">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            ${isAdmin ? `<button id="btn-add-prod" class="bg-white border border-green-600 text-green-700 px-3 py-1.5 rounded-lg text-[10px] font-bold shadow-sm active:scale-95">+ THÊM MÃ</button>` : ''}
                        </div>
                        <div class="space-y-3">
                            <div class="flex gap-2">
                                ...
                                <div class="flex-1"><label class="text-[9px] font-bold text-slate-500 uppercase ml-1">Nguồn Thu</label><select id="h-area" class="w-full p-2 rounded-lg border text-xs font-bold bg-white"><option value="">-- Chọn --</option>${houses.map(h => `<option value="${h.id}" data-name="${h.name}">${h.name}</option>`).join('')}<option value="MuaNgoai" data-name="Mua Ngoài">Mua Ngoài</option></select></div>
                            </div>

                            ${g1.length ? `<div class="bg-green-50 p-2 rounded-xl border border-green-200"><div class="text-[10px] font-bold text-green-700 mb-1 ml-1 uppercase">🍄 Nấm Tươi</div><div class="grid grid-cols-2 gap-2">${g1.map(renderRow).join('')}</div></div>`:''}
                            ${g2.length ? `<div class="bg-orange-50 p-2 rounded-xl border border-orange-200"><div class="text-[10px] font-bold text-orange-700 mb-1 ml-1 uppercase">🍂 Phụ Phẩm</div><div class="grid grid-cols-2 gap-2">${g2.map(renderRow).join('')}</div></div>`:''}
                            ${g3.length ? `<div class="bg-purple-50 p-2 rounded-xl border border-purple-200"><div class="text-[10px] font-bold text-purple-700 mb-1 ml-1 uppercase">🏭 Sơ Chế</div><div class="grid grid-cols-2 gap-2">${g3.map(renderRow).join('')}</div></div>`:''}

                            <button id="btn-save-h" class="w-full py-3 bg-green-600 text-white rounded-xl font-bold text-xs shadow-lg mt-2 active:scale-95 transition">LƯU KHO</button>
                            
                            <div class="mt-4 pt-2 border-t border-green-200">
                                <span class="text-[9px] font-bold text-slate-400 uppercase ml-1">Nhật ký nhập (Gần nhất)</span>
                                <div class="max-h-40 overflow-y-auto space-y-1 mt-1 bg-white p-2 rounded-lg border border-green-100 shadow-inner">
                                    ${sortedHarvest.length ? sortedHarvest.map(l=>`<div class="flex justify-between text-[10px] border-b border-dashed border-slate-100 pb-1 mb-1"><div><span class="text-slate-400 mr-1">${new Date(l.time).toLocaleDateString('vi-VN').slice(0,5)}</span><span class="font-bold text-slate-600">${l.area}</span></div><span class="font-bold text-green-600">+${l.total}kg</span></div>`).join('') : '<div class="text-center text-[10px] text-slate-300 italic">Chưa có dữ liệu</div>'}
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                <div id="zone-sell" class="hidden animate-pop">
                    <div class="glass p-5 border-l-8 border-orange-500 bg-orange-50/40">
                        <div class="flex justify-between items-center mb-4"><h3 class="font-black text-orange-800 text-xs uppercase flex items-center gap-2"><i class="fas fa-truck text-lg"></i> XUẤT BÁN</h3>${isAdmin ? `<button id="btn-rep-day" class="bg-white border text-orange-700 px-3 py-1 rounded-lg text-[10px] font-bold shadow-sm hover:bg-orange-50">Báo Cáo</button>` : ''}</div>
                        <div class="space-y-3 bg-white p-3 rounded-xl shadow-sm border border-orange-100">
                            <div class="grid grid-cols-1 gap-2"><input id="s-cust" placeholder="Tên Khách Hàng" class="w-full p-2 text-xs border rounded-lg font-bold"><textarea id="s-note" placeholder="Ghi chú đơn hàng..." class="w-full p-2 text-xs border rounded-lg h-10"></textarea></div>
                            <div class="bg-slate-50 p-2 rounded-lg border border-slate-200"><div class="flex gap-2 mb-2"><select id="s-prod" class="flex-1 p-2 text-xs border rounded-lg font-bold bg-white"><option value="">-- Chọn Nấm --</option>${products.map(p => `<option value="${p.code}" data-name="${p.name}" data-price="${p.price||0}">${p.name} (Tồn: ${p.stock||0})</option>`).join('')}</select></div><div class="flex gap-2 items-center"><input id="s-qty" type="number" placeholder="SL" class="w-16 p-2 text-xs border rounded-lg text-center font-bold"><span class="text-xs">x</span><input id="s-price" type="number" placeholder="Giá" class="w-20 p-2 text-xs border rounded-lg text-center"><button id="btn-add-cart" class="flex-1 bg-orange-500 text-white p-2 rounded-lg font-bold text-xs shadow active:scale-95">THÊM</button></div></div>
                            <div class="border-t pt-2"><div id="cart-list" class="space-y-1 mb-2 text-xs max-h-32 overflow-y-auto"></div><div class="flex justify-between text-sm font-black text-orange-800 bg-orange-50 p-2 rounded-lg"><span>TỔNG TIỀN:</span><span id="cart-total">0đ</span></div></div>
                        </div>
                        <div class="grid grid-cols-2 gap-2 mt-4"><button id="btn-save-sell" class="py-3 bg-orange-600 text-white rounded-xl font-bold text-xs shadow active:scale-95">LƯU & TRỪ KHO</button><button id="btn-print" class="py-3 bg-blue-600 text-white rounded-xl font-bold text-xs shadow active:scale-95">IN PHIẾU</button></div>
            ...
            setTimeout(() => {
                ...
                if(isAdmin) {
                    ...
                    if(btnAddP) btnAddP.onclick = () => {
                        Utils.modal("Thêm Mã Mới",`<input id="n-n" placeholder="Tên SP (VD: Nấm Hương)" class="w-full p-2 border rounded mb-2"><input id="n-c" placeholder="Mã (Viết liền: namhuong)" class="w-full p-2 border rounded mb-2"><select id="n-g" class="w-full p-2 border rounded"><option value="1">Nấm Tươi</option><option value="2">Phụ Phẩm</option><option value="3">Sơ Chế</option></select>`,[{id:'s-p',text:'Lưu Mã'}]);
                        setTimeout(() => document.getElementById('s-p').onclick = async () => {
                            ...
                            if (n && c) {
                                ...
                                Utils.modal(null); 
                                Utils.toast("✅ Đã thêm mã mới!");
                            }
                        }, 100);
                    };
                    const btnRep = document.getElementById('btn-report-day');
                    if(btnRep) btnRep.onclick=()=>{
                        if(!sortedShips.length)return Utils.toast("Chưa có đơn!","err");
                        let csv="Ngay,Khach,SP,SL,Gia,ThanhTien\n"; sortedShips.forEach(l=>{const d=new Date(l.time).toLocaleDateString('vi-VN');l.items.forEach(i=>csv+=`${d},${l.customer},${i.name},${i.qty},${i.price},${i.qty*i.price}\n`)});
