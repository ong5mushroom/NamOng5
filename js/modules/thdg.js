...
window.THDG_Action = {
    ...
    // Xóa tất cả (RESET) - PHIÊN BẢN AN TOÀN (KHÔNG CẦN BATCH)
    resetAll: async () => {
        // Lấy danh sách đang hiển thị
        const prods = window._CURRENT_PRODUCTS || [];
        
        if (prods.length === 0) return Utils.toast("Danh sách trống!");

        const code = prompt(`⚠️ CẢNH BÁO!\nBạn đang xóa ${prods.length} mã sản phẩm.\nNhập 'OK' để xóa sạch:`);
        
        if (code === 'OK') {
            try {
                Utils.toast("Đang xóa... Vui lòng chờ!");
                // Xóa lần lượt từng cái (An toàn với mọi phiên bản Config)
                const deletePromises = prods.map(p => deleteDoc(doc(db, `${ROOT_PATH}/products`, p._id)));
                await Promise.all(deletePromises);
                
                Utils.toast("✅ Đã xóa sạch!");
                setTimeout(() => location.reload(), 1000); // Tải lại trang để làm mới
            } catch (e) { 
                alert("Lỗi: " + e.message); 
            }
        }
...
export const THDG = {
    render: (data, user) => {
        ...
        const renderRow = (p) => `
            <div id="row-${p._id}" class="flex justify-between items-center bg-white p-2 rounded border border-slate-200 mb-1 shadow-sm">
                <div class="flex items-center gap-2 overflow-hidden">
                    ${isAdmin ? `<button onclick="window.THDG_Action.delOne('${p._id}', '${p.name}')" class="text-red-500 font-bold px-1 text-lg leading-none hover:bg-red-50">×</button>` : ''}
                ...
                <div class="glass p-4 border-l-8 border-green-500 bg-green-50/40">
                    <div class="flex justify-between items-center mb-4">
                        ...
                        <div class="flex gap-2">
                            <button onclick="window.THDG_Action.resetAll()" class="bg-red-100 text-red-600 px-2 py-1 rounded text-[10px] font-bold border border-red-200">RESET TOÀN BỘ</button>
                        ...
                        ${Object.keys(groups).map(k => `
                            <div class="bg-white/60 p-2 rounded-xl border border-slate-200">
                                ...
                                <div class="grid grid-cols-1 gap-1">
                                    ${groups[k].items.length ? groups[k].items.map(renderRow).join('') : '<div class="text-[10px] text-slate-300 italic text-center">Chưa có mã</div>'}
                                </div>
                            </div>
                        `).join('')}

                        <button id="btn-save-h" class="w-full py-3 bg-green-600 text-white rounded-lg font-bold text-xs shadow-lg mt-2 active:scale-95">LƯU KHO</button>
                    </div>
                </div>
            </div>
            ...
            <div id="zone-sell" class="hidden animate-pop">
                <div class="glass p-4 border-l-8 border-orange-500 bg-orange-50/40">
                    ...
                    <div class="space-y-3 bg-white p-3 rounded shadow-sm border border-orange-100">
                        <input id="s-cust" placeholder="Tên Khách Hàng" class="w-full p-2 text-xs border rounded font-bold">
                        <div class="flex gap-2"><select id="s-prod" class="flex-1 p-2 text-xs border rounded font-bold"><option value="">-- Chọn Nấm --</option>${products.map(p => `<option value="${p.code}" data-name="${p.name}" data-price="${p.price||0}">${p.name} (Tồn: ${p.stock||0})</option>`).join('')}</select></div>
                        ...
                        <div id="cart-list" class="border-t pt-2 space-y-1"></div>
        ...
        setTimeout(() => {
            ...
            if(isAdmin) {
                ...
                if(btnAdd) btnAdd.onclick = () => {
                    Utils.modal("Thêm Mã Mới", 
                        `<input id="n-n" placeholder="Tên (VD: Nấm Hương)" class="w-full p-2 border rounded mb-2">
                         <input id="n-c" placeholder="Mã (Viết liền: namhuong)" class="w-full p-2 border rounded mb-2">
            ...
            document.getElementById('btn-save-h').onclick = async () => {
                ...
                // Lưu thủ công không dùng batch để tránh lỗi db.batch
                let totalKg = 0;
                let details = {};
                ...
                for (const p of products) {
                    ...
                    if(el && Number(el.value) > 0) {
                        ...
                        await updateDoc(doc(db, `${ROOT_PATH}/products`, p._id), {stock: increment(q)});
                        details[p.code] = q;
                        totalKg += q;
                        el.value = ''; 
                    }
                ...
                if(totalKg > 0) {
                    ...
                    await addDoc(collection(db, `${ROOT_PATH}/harvest_logs`), {area: aname, details, total: totalKg, user: user.name, time: new Date(dVal).getTime()});
                    Utils.toast(`Đã lưu ${totalKg}kg!`);
                } else { Utils.toast("Chưa nhập số!", "err"); }
            };
            
            let cart=[]; 
            const upC=()=>{document.getElementById('cart-list').innerHTML=cart.map((i,x)=>`<div class="flex justify-between text-[10px] bg-slate-50 p-1 rounded mb-1"><span>${i.name}</span><span>${i.qty}x${i.price}=${(i.qty*i.price).toLocaleString()} <b onclick="document.getElementById('d-${x}').click()" class="text-red-500 cursor-pointer ml-1">x</b></span><button id="d-${x}" class="hidden"></button></div>`).join(''); document.getElementById('cart-total').innerText=cart.reduce((a,b)=>a+b.qty*b.price,0).toLocaleString()+'đ'; cart.forEach((_,i)=>document.getElementById(`d-${i}`).onclick=()=>{cart.splice(i,1);upC()})};
            ...
            document.getElementById('btn-save-sell').onclick=async()=>{if(cart.length){ await addDoc(collection(db,`${ROOT_PATH}/shipping`),{customer:document.getElementById('s-cust').value,items:cart,total:cart.reduce((a,b)=>a+b.qty*b.price,0),user:user.name, time:Date.now()}); for(const i of cart){const p=products.find(x=>x.code===i.code);if(p) await updateDoc(doc(db,`${ROOT_PATH}/products`,p._id),{stock:increment(-i.qty)})} Utils.toast("Đã xuất bán!"); cart=[]; upC(); document.getElementById('s-cust').value='';} else {Utils.toast("Giỏ trống!","err")} };
