...
export const THDG = {
    render: (data, user) => {
        // --- DEBUG: Kiểm tra xem có tìm thấy thẻ view-th không ---
        const c = document.getElementById('view-th');
        if (!c) {
            console.error("LỖI NGHIÊM TRỌNG: Không tìm thấy thẻ <div id='view-th'> trong index.html");
            return;
        }
        
        // Nếu thẻ đang ẩn thì không làm gì cả
        if (c.classList.contains('hidden')) return;

        console.log("Đang vẽ THDG..."); // Nếu thấy dòng này trong console thì code đã chạy tới đây
        ...
        try {
                ...
                <div id="zone-harvest" class="animate-pop">
                    <div class="glass p-5 border-l-8 border-green-500 bg-green-50/40">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="font-black text-green-800 uppercase text-xs flex items-center gap-2"><i class="fas fa-warehouse text-lg"></i> NHẬP KHO</h3>
                            <button id="btn-add-prod" class="bg-white border border-green-200 text-green-700 px-3 py-1 rounded-lg text-[10px] font-bold shadow-sm">+ Mã SP</button>
                        </div>
                        ...
                        <div class="space-y-4">
                            ...
                            ${g1.length ? `<div class="bg-white p-3 rounded-xl border border-green-100 shadow-sm"><div class="text-[10px] font-black text-green-600 uppercase mb-2 border-b pb-1">1. Nấm Tươi</div><div class="grid grid-cols-2 gap-2">${g1.map(p=>`<div class="flex justify-between items-center bg-green-50 p-2 rounded"><span class="text-[10px] font-bold text-slate-600 w-20 truncate">${p.name}</span><input type="number" step="0.1" id="in-${p.code}" class="w-20 p-1 text-center font-bold text-green-700 bg-white border border-green-200 rounded outline-none" placeholder="0"></div>`).join('')}</div></div>`:''}
                            
                            ${g2.length ? `<div class="bg-white p-3 rounded-xl border border-orange-100 shadow-sm"><div class="text-[10px] font-black text-orange-600 uppercase mb-2 border-b pb-1">2. Phụ Phẩm</div><div class="grid grid-cols-2 gap-2">${g2.map(p=>`<div class="flex justify-between items-center bg-orange-50 p-2 rounded"><span class="text-[10px] font-bold text-slate-600 w-20 truncate">${p.name}</span><input type="number" step="0.1" id="in-${p.code}" class="w-20 p-1 text-center font-bold text-orange-700 bg-white border border-orange-200 rounded outline-none" placeholder="0"></div>`).join('')}</div></div>`:''}

                            ${g3.length ? `<div class="bg-white p-3 rounded-xl border border-purple-100 shadow-sm"><div class="text-[10px] font-black text-purple-600 uppercase mb-2 border-b pb-1">3. Sơ Chế</div><div class="grid grid-cols-2 gap-2">${g3.map(p=>`<div class="flex justify-between items-center bg-purple-50 p-2 rounded"><span class="text-[10px] font-bold text-slate-600 w-20 truncate">${p.name}</span><input type="number" step="0.1" id="in-${p.code}" class="w-20 p-1 text-center font-bold text-purple-700 bg-white border border-purple-200 rounded outline-none" placeholder="0"></div>`).join('')}</div></div>`:''}
                ...
                <div id="zone-sell" class="hidden animate-pop">
                    <div class="glass p-5 border-l-8 border-orange-500 bg-orange-50/40">
                        ...
                        <div class="grid grid-cols-2 gap-2 mt-4">
                            ...
                            <button id="btn-print" class="py-3 bg-blue-600 text-white rounded-xl font-bold text-xs shadow-lg active:scale-95 transition">IN HÓA ĐƠN</button>
                        </div>
                    </div>
                </div>
            </div>`;
            ...
            setTimeout(() => {
                ...
                document.getElementById('btn-save-h').onclick=async()=>{
                    const aid=document.getElementById('h-area').value; if(!aid) return Utils.toast("Chọn Nguồn Thu!","err");
                    ...
                    batch.set(doc(collection(db,`${ROOT_PATH}/harvest_logs`)),{area:aname,details:d,total:t,user:user.name,time:Date.now()});
                    if(aid!=='MuaNgoai') batch.update(doc(db,`${ROOT_PATH}/houses`,aid),{totalYield:increment(t)});
                    await batch.commit(); Utils.toast(`Đã nhập ${t}kg`);
                };
                ...
                document.getElementById('btn-print').onclick=()=>{
                    ...
                    w.document.write(`<html><head><title>HOA DON</title><style>body{font-family:monospace;padding:20px;font-size:12px}.c{text-align:center}.r{text-align:right}table{width:100%;border-collapse:collapse}td,th{border-bottom:1px dashed #000;padding:5px 0}</style></head><body><div class="c"><h3>${COMPANY.name}</h3><p>${COMPANY.address}</p><p>MST:${COMPANY.mst} Hotline:${COMPANY.hotline}</p><br><h2>HÓA ĐƠN</h2></div><p>Khách:${document.getElementById('s-cust').value}</p><table><tr><th>SP</th><th>SL</th><th class="r">Tiền</th></tr>${cart.map(i=>`<tr><td>${i.name}</td><td class="c">${i.qty}</td><td class="r">${(i.qty*i.price).toLocaleString()}</td></tr>`).join('')}<tr><td colspan="3" class="r"><b>TỔNG:${cart.reduce((a,b)=>a+b.qty*b.price,0).toLocaleString()}</b></td></tr></table></body></html>`);
