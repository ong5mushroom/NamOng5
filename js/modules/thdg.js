...
window.THDG_Delete = async (id, name) => {
    // 1. Hỏi xác nhận
    if (confirm(`⚠️ ADMIN: Xóa mã "${name}"?\nTồn kho của mã này sẽ mất.`)) {
        try {
            // 2. Xóa ngay trên màn hình để cảm giác nhanh (Optimistic UI)
            const row = document.getElementById(`row-${id}`);
            if (row) row.style.opacity = '0.3'; // Làm mờ đi trong lúc chờ

            // 3. Gửi lệnh xóa vào Database
            await deleteDoc(doc(db, `${ROOT_PATH}/products`, id));
            Utils.toast("Đã xóa xong!");
            
            // (Lưu ý: Không cần remove() thủ công nữa, onSnapshot trong app.js sẽ tự vẽ lại danh sách mới)
        } catch (e) {
            alert("Lỗi xóa: " + e.message);
            if (row) row.style.opacity = '1'; // Nếu lỗi thì hiện lại
        }
    }
};
...
export const THDG = {
    render: (data, user) => {
        ...
        try {
            ...
            // --- 1. SẮP XẾP DANH SÁCH (QUAN TRỌNG ĐỂ KHÔNG BỊ NHẢY) ---
            // Sort A->Z theo tên sản phẩm
            const products = (Array.isArray(data.products) ? data.products : []).sort((a,b) => (a.name || "").localeCompare(b.name || ""));
            
            const houses = Array.isArray(data.houses) ? data.houses : [];
            
            // Logs và Ships (Mới nhất lên đầu)
            ...
            // Phân nhóm (Dữ liệu đã được sort ở trên nên các nhóm này cũng sẽ được sort)
            ...
            // --- 2. RENDER DÒNG ---
            const renderRow = (p) => `
                <div id="row-${p._id}" class="flex justify-between items-center bg-white p-1.5 rounded border border-slate-200 mb-1">
                    <div class="flex items-center gap-1 overflow-hidden">
                        ${isAdmin ? `
                            <button onclick="window.THDG_Delete('${p._id}', '${p.name}')" class="text-red-400 hover:text-red-600 w-5 h-5 flex items-center justify-center font-bold text-lg" title="Xóa">
                                ×
                            </button>` : ''}
                ...
                <div id="zone-harvest" class="animate-pop">
                    <div class="glass p-5 border-l-8 border-green-500 bg-green-50/40">
                        <div class="flex justify-between items-center mb-4">
                            ...
                            ${isAdmin ? `<button id="btn-add-prod" class="bg-white border border-green-200 text-green-700 px-2 py-1 rounded text-[10px] font-bold shadow-sm hover:bg-green-50">+ Mã SP</button>` : ''}
                        </div>
            ...
            setTimeout(() => {
                ...
                document.getElementById('btn-print').onclick=()=>{if(!cart.length)return Utils.toast("Giỏ trống!","err");const w=window.open('','','height=600,width=500');w.document.write(`<html><body><h2>HÓA ĐƠN</h2><p>Khách:${document.getElementById('s-cust').value}</p><table>${cart.map(i=>`<tr><td>${i.name}</td><td>${i.qty}</td><td>${(i.qty*i.price).toLocaleString()}</td></tr>`).join('')}</table></body></html>`);w.document.close();w.print();};
