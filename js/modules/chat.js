...
export const Chat = {
    render: (data, user) => {
        ...
        try {
            ...
            if (msgs.length === 0) {
                content.innerHTML = `<div class="text-center mt-20 text-slate-400 font-bold opacity-60">Chưa có tin nhắn</div>`;
            } else {
    ...
    sendMessage: async (user) => {
        ...
        if(!text) return;
        inp.value = ''; inp.focus();
        try { await addDoc(collection(db, `${ROOT_PATH}/chat`), { text: text, senderId: String(user._id || user.id), senderName: user.name, time: Date.now() }); } catch (e) { Utils.toast("Lỗi gửi!", "err"); }
    }
};
