...
const App = {
    ...
    init: async () => {
        console.log("App V4 Simple Starting...");
        ...
        if (App.user) {
            ...
            cols.forEach(key => {
                ...
                onSnapshot(collection(db, `${ROOT_PATH}/${key}`), (snap) => {
                    ...
                    if(!document.getElementById('chat-layer').classList.contains('hidden')) Chat.render(App.data, App.user);
                });
            });
        }
    },
    // ... (Giữ nguyên các phần còn lại: renderLoginList, checkLoginState, login, switchTab...)
    renderLoginList: () => {
    ...
    checkLoginState: () => {
        if (App.user) {
            ...
            if(['admin', 'quản lý', 'giám đốc'].some(r => (App.user.role||'').toLowerCase().includes(r))) document.getElementById('btn-settings').classList.remove('hidden');
            App.switchTab('home');
    ...
    login: () => {
        ...
        if (u && String(u.pin) === String(pin)) { App.user = u; localStorage.setItem('n5_modular_user', JSON.stringify(u)); location.reload(); } 
    ...
    switchTab: (tab) => {
        ...
        if(view) { view.classList.remove('hidden'); document.querySelector(`.nav-btn[data-tab="${tab}"]`).classList.add('active', 'text-blue-600'); App.currentTab = tab; App.renderActiveTab(); }
    },
    renderActiveTab: () => {
        if(!App.user) return;
        try {
    ...
    bindEvents: () => {
        ...
        document.getElementById('btn-chat').onclick = () => { document.getElementById('chat-layer').classList.remove('hidden'); Chat.render(App.data, App.user); };
