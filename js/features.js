...
export const Features = {
    ...
    Warehouse: {
        ...
        actions: {
            ...
            submitShip: async (user) => { 
                const c = document.getElementById('ship-cust').value; const q = Number(document.getElementById('ship-qty').value); 
    ...
    HR: {
        renderTasks: (data, user) => {
            ...
            container.innerHTML = `
            <div class="space-y-6">
                 ${canAssign ? `<div class="glass p-5 border-l-4 border-blue-500"><h4 class="font-black text-slate-700 uppercase text-xs mb-3">Giao Việc (Nhiều người)</h4><div class="space-y-3"><input id="task-title" placeholder="Tên công việc"><div class="grid grid-cols-2 gap-3"><select id="task-house"><option value="">-- Nhà/Khu --</option>${data.houses.map(h=>`<option value="${h.name}">${h.name}</option>`).join('')}</select><input id="task-deadline" type="date"></div><div class="max-h-32 overflow-y-auto border border-slate-200 rounded-lg p-2 grid grid-cols-2 gap-2">${empCheckboxes}</div><textarea id="task-desc" placeholder="Mô tả..."></textarea><button class="w-full py-3 bg-blue-600 text-white rounded-xl font-bold btn-action" data-action="addTask">PHÁT LỆNH</button></div></div>` : ''}
