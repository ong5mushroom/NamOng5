...
const App = {
    ...
    init: () => {
        ...
        signInAnonymously(auth).then(() => {
            document.getElementById('login-status').innerHTML = '<span class="text-green-500">✔ Đã kết nối V226</span>';
            App.syncData();
            if(App.user) {
    ...
    syncData: () => {
        ...
        colls.forEach(c => {
            onSnapshot(collection(db, `${ROOT_PATH}/${c}`), (snapshot) => {
                ...
                if(c === 'employees') {
                    if(snapshot.empty) { // Auto-seed Giám đốc nếu trống
