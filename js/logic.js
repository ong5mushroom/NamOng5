...
const App = {
    ...
    syncData: () => {
        ...
        colls.forEach(c => {
            onSnapshot(collection(db, `${ROOT_PATH}/${c}`), (snap) => {
                ...
                if(c==='employees' && snap.empty) addDoc(collection(db, `${ROOT_PATH}/employees`), { id: 9999, name: "Giám Đốc", pin: "9999", role: "Giám đốc", score: 100 });
    ...
    actions: {
        ...
        addTask: async () => { const t = document.getElementById('task-title').value; const h = document.getElementById('task-house').value; const a = document.getElementById('task-assignee').value; const d = document.getElementById('task-deadline').value; const desc = document.getElementById('task-desc').value; if(!t || !a) return alert("Thiếu tin!"); await addDoc(collection(db, `${ROOT_PATH}/tasks`), { title:t, house:h, assignee:a, deadline:d, desc, status:'pending', createdBy:App.user.name, time:Date.now() }); App.helpers.notify(`📋 Giao việc: ${t} cho ${a}`); },
        receiveTask: async (id) => { await updateDoc(doc(db, `${ROOT_PATH}/tasks`, id), {status:'received', receivedAt:Date.now()}); App.helpers.notify(`💪 ${App.user.name} đã nhận việc`); },
        submitTask: async (id) => { const todayTasks = App.data.tasks.filter(t => t.assignee === App.user.name && new Date(t.time).getDate() === new Date().getDate()); const points = todayTasks.length > 0 ? (10 / todayTasks.length) : 10; const empRef = App.data.employees.find(e => e.id === App.user.id); if(empRef) { await updateDoc(doc(db, `${ROOT_PATH}/employees`, empRef._id), { score: Math.round(((empRef.score||0) + points) * 10) / 10 }); } await updateDoc(doc(db, `${ROOT_PATH}/tasks`, id), {status:'done', completedBy:App.user.name, completedAt:Date.now()}); App.helpers.notify(`✅ ${App.user.name} xong việc (+${points.toFixed(1)}đ)`); },
        ...
        submitTH: async () => {
            ...
            App.helpers.notify(`🍄 ${App.user.name} nhập ${total}kg từ ${area}`);
        },
