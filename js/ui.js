export const UI = {
    ...
    renderHome: (houses, harvestLogs) => {
        ...
        container.innerHTML = `
        <div class="p-2 pb-20">
            <h2 class="text-lg font-black text-slate-700 mb-3 uppercase border-b-2 border-blue-500 inline-block">Trạng Thái Trại (V222)</h2>
            ...
            ${sortedHouses.map(h => {
                ...
                <div class="card p-3 relative overflow-hidden group border-l-4 ${h.status === 'ACTIVE' ? 'border-green-500' : 'border-slate-300'}">
                    <div class="flex justify-between items-start mb-2">
                        <div><h3 class="font-black text-xl text-blue-900">${h.name}</h3><div class="text-[10px] text-slate-500 font-bold uppercase">${h.currentBatch ? h.currentBatch : '(Trống)'}</div></div>
                        <span class="text-[9px] ${h.status==='ACTIVE'?'bg-green-100 text-green-700':'bg-slate-100 text-slate-500'} px-1 py-0.5 rounded font-bold">${h.status === 'ACTIVE' ? 'SX' : 'CHỜ'}</span>
                    </div>
    ...
    renderSX: (houses) => {
        ...
        // Sort logic lặp lại để đảm bảo đồng bộ
        const houseOrder = ['A', 'A+', 'B1', 'B2', 'C1', 'C2', 'C3', 'C4', 'C5', 'C6', 'D1', 'D2', 'D3', 'D4', 'E1', 'E2', 'E3', 'E4', 'E5', 'F'];
        ...
        <div class="p-2 space-y-4">
            <div class="card border-2 border-blue-500 shadow-lg">
                ...
                <div class="p-4 space-y-3">
                    <div><label class="block text-xs font-bold text-slate-500 mb-1">CHỌN NHÀ / KHO A</label><select id="sx-house-select" class="input-box font-bold text-blue-800">${sortedHouses.map(h => `<option value="${h._id}">${h.name}</option>`).join('')}</select></div>
                    <div class="grid grid-cols-2 gap-3">
                ...
                <div class="card border-2 border-green-500 shadow-xl mb-4">
                    ...
                    <div class="p-3 space-y-3">
                        ...
                        <div class="bg-green-50 p-2 rounded border border-green-200">
                            ...
                            <div class="grid grid-cols-2 gap-2">${groupStd.map(m => `<div class="flex items-center gap-1"><label class="w-16 text-xs font-bold text-slate-600">${m.l}</label><input type="number" step="0.1" id="th-${m.c}" class="input-harvest flex-1 input-box !p-1 font-bold text-slate-800 text-center" placeholder="-"></div>`).join('')}</div>
                        </div>
                        <div class="bg-orange-50 p-2 rounded border border-orange-200">
                            ...
                            <div class="grid grid-cols-2 gap-2">${groupLimit.map(m => `<div class="flex items-center gap-1"><label class="w-16 text-xs font-bold text-slate-600">${m.l}</label><input type="number" step="0.1" id="th-${m.c}" class="input-harvest flex-1 input-box !p-1 font-bold text-slate-800 text-center" placeholder="-"></div>`).join('')}</div>
                        </div>
    ...
    renderStock: (inv, supplies, distributionLogs) => { 
        ...
        <div class="p-2 space-y-4">
            <div class="card border-2 border-purple-500 shadow-md">
                 ...
                 <div class="p-3 text-center"><p class="text-xs text-slate-500">Quản lý nhập xuất Phôi tại Tab <b>SX</b></p></div>
            </div>
            <div class="card border-2 border-orange-500 shadow-lg">
                 <div class="bg-orange-600 text-white p-2 font-bold uppercase rounded-t flex justify-between"><span><i class="fas fa-box-open mr-2"></i>Kho THDG (Nấm)</span></div>
                 <div class="p-3">
    ...
    // --- 6. TAB VIỆC (CARD + CHỌN NHÀ + 3 TRẠNG THÁI) ---
    renderTasksAndShip: (tasks, currentUser, houses) => {
        ...
        <div class="p-2 space-y-4">
             ...
             <div class="card p-3 border-2 border-blue-400 bg-blue-50 mb-4 shadow">
                ...
                <div class="mb-2"><select id="task-house" class="input-box w-full text-sm"><option value="">-- Chọn Khu vực --</option>${houses.map(h => `<option value="${h.name}">${h.name}</option>`).join('')}</select></div>
                 ...
                 ${myTasks.map(t => {
                    ...
                    <div class="card p-4 border-l-4 ${t.status==='received'?'border-blue-500':'border-red-500'} shadow-md bg-white">
                        <div class="flex justify-between items-start">
                            <div><h4 class="font-bold text-slate-800 text-lg">${t.title}</h4><div class="text-xs font-bold text-blue-600 bg-blue-50 px-2 py-1 rounded inline-block mt-1">${t.house || 'Chung'}</div></div>
                            <span class="text-[10px] ${t.status==='received'?'bg-blue-100 text-blue-600':'bg-red-100 text-red-600'} px-2 py-1 rounded font-bold">${t.status==='received' ? 'ĐANG LÀM' : 'CHƯA NHẬN'}</span>
                        </div>
    ...
    renderChat: (messages, currentUserId) => {
        ...
        box.innerHTML = messages.map(m => {
            ...
            if(m.type === 'system') return `<div class="flex justify-center my-2"><div class="bg-slate-200 text-slate-600 text-[10px] px-3 py-1 rounded-full font-bold uppercase shadow-sm">${m.text}</div></div>`;
