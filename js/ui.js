export const UI = {
    ...
    renderHome: (houses, harvestLogs, employees) => {
        ...
        <div class="space-y-6">
            ...
            <div>
                <h2 class="text-sm font-black text-slate-600 uppercase mb-3 pl-3 border-l-4 border-blue-600">Khu Vực Sản Xuất</h2>
                <div class="grid grid-cols-2 gap-3">
                ${sortedHouses.map(h => `
                    <div class="card p-3 border border-slate-100">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                <h3 class="font-black text-lg text-slate-800">${h.name} ${h.name==='A'?'<span class="text-[8px] text-purple-600 bg-purple-50 px-1 rounded ml-1">KHO PHÔI</span>':''}</h3>
                        ...
                        <div class="text-right border-t pt-1 mt-1 border-slate-50">
                            <span class="text-[10px] text-slate-400 mr-1">Thu:</span>
             ...
             <div class="card p-5 border border-blue-50">
                ...
                <div class="space-y-3">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        <button class="btn-primary bg-blue-600 rounded-lg shadow font-bold text-xs hover:bg-blue-700 active:scale-95 transition h-full" data-action="addTask">PHÁT LỆNH</button>
                    </div>
             ...
             <div>
                 ...
                 <div class="space-y-3">
                     ${myTasks.length ? myTasks.map(t => `
                        <div class="card p-4 border-l-4 ${t.status==='received'?'border-blue-500':'border-red-500'} relative">
                            <div class="flex justify-between items-start mb-2">
    ...
    // --- 3. THDG: ĐỦ MÃ NẤM & TÍNH CHÊNH LỆCH KHO ---
    renderTH: (houses, harvestLogs, shippingLogs, supplies) => {
        ...
        <div class="space-y-4">
            <div class="flex gap-2 bg-white p-1.5 rounded-xl shadow-sm border border-slate-200">
                <button class="flex-1 py-2 rounded-lg font-bold text-xs bg-green-100 text-green-700 shadow-sm" onclick="document.getElementById('zone-th').classList.remove('hidden');document.getElementById('zone-ship').classList.add('hidden');">NHẬP</button>
                <button class="flex-1 py-2 rounded-lg font-bold text-xs text-slate-400 hover:bg-slate-50" onclick="document.getElementById('zone-th').classList.add('hidden');document.getElementById('zone-ship').classList.remove('hidden');">XUẤT & TỒN</button>
            </div>
                ...
                <div class="card p-5 border border-green-100">
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200">
                            ...
                            <div class="grid grid-cols-3 gap-3">${g1.map(m=>`<div><label class="text-[9px] font-bold text-slate-400 block mb-1">${m.l}</label><input type="number" step="0.1" id="th-${m.c}" class="input-harvest w-full text-center font-bold text-sm focus:border-green-500" placeholder="-"></div>`).join('')}</div>
                        </div>
                        
                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200">
                            <h4 class="text-[10px] font-bold text-slate-500 uppercase mb-2">2. Phụ Phẩm (Kg)</h4>
                            <div class="grid grid-cols-3 gap-3">${g2.map(m=>`<div><label class="text-[9px] font-bold text-slate-400 block mb-1">${m.l}</label><input type="number" step="0.1" id="th-${m.c}" class="input-harvest w-full text-center font-bold text-sm focus:border-orange-500" placeholder="-"></div>`).join('')}</div>
                        </div>

                        <div class="bg-slate-50 p-3 rounded-xl border border-slate-200">
                            <h4 class="text-[10px] font-bold text-slate-500 uppercase mb-2">3. Thành Phẩm (Gói)</h4>
                            <div class="grid grid-cols-3 gap-3">${g3.map(m=>`<div><label class="text-[9px] font-bold text-slate-400 block mb-1">${m.l}</label><input type="number" id="th-${m.c}" class="input-harvest w-full text-center font-bold text-sm focus:border-purple-500" placeholder="-"></div>`).join('')}</div>
                        </div>
            ...
            <div id="zone-ship" class="hidden">
                ...
                <div class="mt-4 card p-4 border border-slate-200">
                    ...
                    <div class="flex gap-2 items-end">
                        <div class="flex-1"><label class="text-[9px] text-slate-400 font-bold">TỒN MÁY TÍNH</label><div class="bg-slate-100 p-3 rounded-lg text-slate-500 font-bold text-sm border">150.0</div></div>
        ...
        container.innerHTML = `<div class="p-4 space-y-4">
            ...
            <div class="grid grid-cols-2 gap-3 mb-6">
                <button class="card p-4 flex flex-col items-center gap-2 btn-action active:bg-green-50 transition" data-action="submitAttendance">
                    <div class="w-12 h-12 bg-green-100 rounded-full flex items-center justify-center text-green-600 text-xl"><i class="fas fa-fingerprint"></i></div><span class="text-xs font-black text-slate-700">ĐIỂM DANH</span>
                </button>
                <button class="card p-4 flex flex-col items-center gap-2 btn-action active:bg-orange-50 transition" data-action="toggleModal" data-payload="modal-leave">
                    <div class="w-12 h-12 bg-orange-100 rounded-full flex items-center justify-center text-orange-600 text-xl"><i class="fas fa-file-signature"></i></div><span class="text-xs font-black text-slate-700">XIN NGHỈ</span>
                </button>
                <button class="card p-4 flex flex-col items-center gap-2 btn-action active:bg-purple-50 transition" data-action="toggleModal" data-payload="modal-buy-req">
                    <div class="w-12 h-12 bg-purple-100 rounded-full flex items-center justify-center text-purple-600 text-xl"><i class="fas fa-shopping-basket"></i></div><span class="text-xs font-black text-slate-700">MUA HÀNG</span>
                </button>
                <button class="card p-4 flex flex-col items-center gap-2 btn-action active:bg-slate-100 transition" data-action="logout">
                    <div class="w-12 h-12 bg-slate-200 rounded-full flex items-center justify-center text-slate-600 text-xl"><i class="fas fa-power-off"></i></div><span class="text-xs font-black text-slate-700">THOÁT</span>
                </button>
            </div>
    ...
    // --- CÀI ĐẶT & CHAT ---
    renderSettingsModal: (employees) => { /* Giữ nguyên V300 */ },
    renderChat: (msgs, uid) => {
    ...
    // GIỮ NGUYÊN CÁC HÀM SX, RENDER SX VÀ SETTINGS TỪ BẢN TRƯỚC
    renderSX: (houses) => { 
    ...
    renderSettingsModal: (employees) => { /* Code giống V300 */ }
};
