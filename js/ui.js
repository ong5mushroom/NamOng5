// MODULE GIAO DIỆN (VIEW) - V163 FULL
export const UI = {
    showMsg: (t) => {
    ...
    // 1. RENDER HOME
    renderHome: (houses, harvest, employees) => {
        ...
        const houseList = houses.map(h => {
            return `<div class="card border-l-4 border-amber-800 shadow-sm flex justify-between items-center">
                <span class="font-black text-sm uppercase">${h.name}</span>
                <button class="text-[9px] bg-slate-100 border px-2 py-1 rounded font-bold btn-action" data-action="exportCSVByHouse" data-payload="${h.name}">NHẬT KÝ</button>
            </div>`;
        }).join('');
        ...
        container.innerHTML = `
            ...
            <p class="label px-2">Danh sách Nhà Nấm</p>
    ...
    renderTasks: (tasks, employees, houses, user) => {
        ...
        let myTasks = tasks.filter(t => {
            ...
            if(t.status === 'completed') return false; // Chỉ hiện việc chưa xong cho gọn
            return true;
        });
    ...
    renderTH: (houses, harvestLogs) => {
        ...
        const tableHtml = logs.map(l => {
            ...
            return `<tr>
                <td class="text-[9px]">${new Date(l.time).toLocaleDateString().slice(0,5)}</td>
                <td class="font-bold text-blue-600">${l.area}</td>
                <td class="font-black text-green-600">${total.toFixed(1)}</td>
                <td class="text-[9px] uppercase">${l.user.split(' ').pop()}</td>
            </tr>`;
        }).join('');
        ...
        container.innerHTML = `
             ...
             <table class="log-table shadow-sm">
                <thead><tr><th>Ngày</th><th>Nhà</th><th>Kg</th><th>NV</th></tr></thead>
